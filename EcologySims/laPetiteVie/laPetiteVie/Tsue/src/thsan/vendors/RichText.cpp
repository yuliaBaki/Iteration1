...
namespace sfe
{
	void RichText::setLineSpacing(float spacing)
	{
		m_lineSpacing = spacing;
		updateGeometry();
	}

	void RichText::Line::setLineSpacing(float lineSpacing)
	{
	...
	float RichText::getLineSpacing(float spacing)
	{
	...
	void RichText::Line::updateTextAndGeometry(sf::Text& text) const
	{
		...
		// Update bounds
		float lineSpacing = std::floor(text.getFont()->getLineSpacing(text.getCharacterSize() / 2.0f)) + m_lineSpacing; // temporary fix. remove 2.0f to go to the original.

		//okay buddy., font stuff is here. You have to learn how a font work and ahow to modify one in file
		//option 1 a lineSpacing var and always modifiy the font so that it fit the biggest font.
		//option 2, custom font that work as intended and you can add a spacing var with ease of mind.
		// the 2.0f division is pur luck with the font. It must be removed eventually (hopefully,I don't know the future).
	...
	void RichText::updateGeometry() const
	{
		...
		for (Line& line : m_lines) {
			...
			m_bounds.height += line.getGlobalBounds().height;
			m_bounds.width = std::max(m_bounds.width, line.getGlobalBounds().width);
			line.setLineSpacing(m_lineSpacing);
		}
	}
